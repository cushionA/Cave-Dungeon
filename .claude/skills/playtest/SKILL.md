         ...
         └─[success]──→ [2. SCENE_BUILD]
                          ...
                          └──→ [2.5. PREFLIGHT (T6)]
                                 ...
                                 └──→ [3. RUN_EDIT_TESTS (T1)]
                                        ...
                                        │                     │ 失敗テストの詳細を分析
                                        │                     └──→ [4. PLAYMODE_SESSION] (続行)
                                        │
                                        └─[all pass]──→ [4. PLAYMODE_SESSION]
                                                         │ ※ PlayMode セッションを1回にまとめる
                                                         │
                                                         │ [4a. CONFIGURE]
                                                         │   unicli exec Scene.Open --path "Assets/Scenes/CoreTestScene.unity"
                                                         │   → GRAPH:AUTO_INPUT_EXECUTE の [1. CONFIGURE] のみ実行（PlayMode Enter はまだ）
                                                         │
                                                         │ [4b. PROFILER_START (T5)]
                                                         │   `/unicli`: Profiler.StartRecording
                                                         │
                                                         │ [4c. ENTER_PLAY + AUTO_INPUT (T2)]
                                                         │   `/unicli`: PlayMode.Enter
                                                         │   → GRAPH:AUTO_INPUT_EXECUTE の [3. WAIT_COMPLETION] へ
                                                         │   ※ PlayMode.Enter/Exit はこのセッションで制御
                                                         │
                                                         │ [4d. IN-PLAY CHECKS] (PlayMode中、AutoInput完了後)
                                                         │   T3 Snapshot: Eval で状態値検証（該当時）
                                                         │   T4 Animator: Animator.Inspect で状態確認（該当時）
                                                         │   T7 Dynamic: シーン操作→検証（該当時）
                                                         │   T8 UI: Eval で UI要素検証（該当時）
                                                         │   T9 Screenshot: Screenshot.Capture（該当時）
                                                         │   ※ 各テスト種別の要否は feature-test-matrix.md 参照
                                                         │   ※ 詳細手順は該当の t*.md を Read
                                                         │
                                                         │ [4e. PROFILER_STOP (T5)]
                                                         ...
                                                         └──→ [5. ANALYZE]
                                                                ...
                                                                │   ※ EditMode で実行（ランタイム状態はステップ4で取得済み）
                                                                │
                                                                └──→ [6. REPORT]
                                                                       │ レポート出力（下記テンプレート参照）
...
[1. CONFIGURE]
  ...
  └──→ [2. ENTER_PLAY]
         ...
         └──→ [3. WAIT_COMPLETION]
                ...
                │
                ├─[unicli応答なし]──→ [CRASH_DETECTED]
                │                      │ unicli check で接続確認
                │                      │ ├─[応答あり]→ PlayMode.Exit → エラー報告
                │                      │ └─[応答なし]→ Unityクラッシュとして報告、ユーザーにEditor再起動を案内
                │                      └──→ END
                │
...
エラーログを分析して問題を分類する。
**実行モード**: EditMode（PlayMode Exit 後に呼ばれる）。ランタイム状態の検証は FULL_WORKFLOW の Step 4d で PlayMode 中に取得済みの結果を使う。

```
[1. COLLECT_LOGS]
  │ `/unicli`: Console.GetLog
  │ Read: auto-input-test-log.txt (存在する場合)
  │ Step 4d の IN-PLAY CHECKS 結果を参照（T3/T4/T7/T8のデータ）
  │
  └──→ [2. INSPECT_SCENE]
         ...
         │ ※ ランタイム状態（HP値、Animator状態等）はStep 4dの結果を使う
         │
         └──→ [3. CATEGORIZE]
                │ エラーを分類:
                │
