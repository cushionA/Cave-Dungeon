...
- [ ] **CompanionAISettings UI: フッターバーのパッド入力配線未実装** — UXML/USS にフッターバー（A/B/X/Y 凡例）を定義済みだが、実際のパッドボタン→UIアクション紐付けがない。誤認識防止に現在は `hidden` クラスで非表示。
  ...
  - 修正手順:
    1. InputSystem の Action Map に CompanionAISettings 用の A/B/X/Y バインド追加
    2. Controller が Input 購読 → Back/Save/SaveAsPreset/Accept を発火
    3. UXML の footer-bar から `hidden` クラスを除去して表示復帰
  - 優先度: 🟡 Medium（UIは配置済み、配線待ち）
