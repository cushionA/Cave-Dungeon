...
- [ ] **CharacterRefs 抜本リファクタ(ハッシュ→キャラ参照の一元化)** — 現状 `SoACharaDataDic._damageables[]` で `IDamageable` のみハッシュ逆引き可能だが、`BaseCharacter`/`ActionExecutorController`/`CharacterAnimationController` 等、他のキャラ紐付きコンポーネントは `GetComponent` でしか取得できない。`CharacterRefs` クラスにまとめて SoA に `_refs[]` として一元管理する
  - 対象:
    - Core 層: `Assets/MyAsset/Core/Common/CharacterRefs.cs`(新設) / `Assets/MyAsset/Core/Common/Interfaces.cs` (IPooledCharacter等追加) / `Assets/MyAsset/Core/DataContainer/SoACharaDataDic.cs`(`_damageables` → `_refs` 置換)
    - Runtime 層: `BaseCharacter`/`DamageReceiver`/`ActionExecutorController`/`CharacterAnimationController`/`CharacterCollisionController` に各インターフェース実装
    - 呼び出し元: `ProjectileHitProcessor`/`ProjectileManager`/`HitBox`/`DamageDealer`/`EnemySpawnerManager.HandleEnemySpawned:279`/`CompanionCharacter.cs:76` 等
  - 設計:
    ```csharp
    // Core
    public class CharacterRefs
    {
        public IPooledCharacter Pooled;         // BaseCharacter
        public IDamageable Damageable;          // DamageReceiver
        public IActionExecutor ActionExecutor;  // ActionExecutorController
        public IAnimationBridge AnimationBridge;// CharacterAnimationController
        public ICollisionController Collision;  // CharacterCollisionController
    }
    // SoACharaDataDic
    private CharacterRefs[] _refs;
    public CharacterRefs GetRefs(int hash);
    public void SetRefs(int hash, CharacterRefs refs);
    ```
  - 方針:
    - **AI関連(AIInfo/AIMode/AIRule/ActionSlot/DamageScoreTracker 等)は現状の値型/SoA管理を維持**。CharacterRefsは MonoBehaviour 参照だけを扱う
    - スポーン直前のタイミング問題: プレハブの `BaseCharacter.Awake` で `CharacterRefs` を自分で組み立てて `SetRefs(hash, ...)` する。以降のコードは全部 `GetRefs(hash).Damageable` 経由でアクセス
    - 既存 `SetManaged`/`GetManaged` は削除、`SetRefs`/`GetRefs` に置換
  - 解決される既存GetComponent:
    - `EnemySpawnerManager.cs:279` (BaseCharacter スポーン時キャッシュ)
    - `CompanionCharacter.cs:76` (プレイヤー参照取得)
    - `BaseCharacter.Start/OnPoolAcquire` のフォールバック GetComponent
  - 優先度: 🟡 Medium（設計整合性向上。毎フレーム呼び出しではないが、SoA の参照管理を統一できる）
...
- [ ] **戦術プリセット切替の別導線（L1/R1+方向キー等）未実装** — ショートカット（単押し）は「現戦術内のモード切替」に特化したため（2026-04-21にセマンティクス変更）、戦術プリセットそのものを戦闘中に切り替える経路が欠落。ボス戦⇔雑魚戦で戦術を丸ごと切り替えたいケースをカバーする
  - 対象: `Assets/MyAsset/Runtime/Input/` 配下の入力処理 / `Assets/MyAsset/Core/AI/Companion/CompanionController.cs` / UI上の凡例表示
  - 設計メモ:
    - ショートカット（単押し十字キー）= 現戦術内のモード切替
    - L1/R1 + 十字キー（2段階入力）= 戦術プリセット切替
    - 戦術切替時はプリセット全体を入替＋新戦術の `shortcutModeBindings` を読み直す
    - UI上に「戦術プリセット切替は L1 + 方向キー」のような操作ガイドを常時表示
  - 優先度: 🟡 Medium（ショートカット意味変更の完結に必要）
...
- [ ] **デフォルト行動「何もしない」Idle アクションの runtime 実装** — Phase 5 UI で DEFAULT 行は「何もしない」固定表示にしたが、内部 `defaultActionIndex` は従来通り任意 ActionSlot を指しうる。既存プリセットで default が非 Idle に設定されていると、UI 表示（何もしない）と runtime 挙動（設定された行動が実行される）に乖離が出る
  - 対象: `Assets/MyAsset/Core/Common/Enums.cs`（SustainedAction 拡張）/ `Assets/MyAsset/Core/AI/RuleBuilder/ActionSlotLabelTable.cs` / `Assets/MyAsset/Core/UI/CompanionAISettingsLogic.cs`（GcOrphanActionSlots 正規化）/ `Assets/MyAsset/Runtime/UI/CompanionAISettingsController.Dialogs.cs`
  - 修正案:
    1. `SustainedAction.Idle`（または `InstantAction.Idle`）を追加し、ハンドラで「現状維持・停止」を実装
    2. `CompanionAISettingsLogic.GcOrphanActionSlots` の最後で、default が Idle 以外を指していれば Idle スロットへ差し替える（既存プリセットの正規化）
    3. `AIMode` 新規作成時に default を必ず Idle で埋める
  - 優先度: 🟡 Medium（Phase 5 の UX 前提を runtime で担保する後続タスク）
...
- [ ] **SoACharaDataDic の固定容量超過検知（PR #29 レビュー）** — 自動生成コンテナは容量超過時に `InvalidOperationException` を投げる。現状 `k_InitialContainerCapacity = 256` だが、ボス戦+弾幕+召喚獣で超える可能性がある
  - 対象: `Assets/MyAsset/Runtime/GameManager.cs:14`
  - 修正案: `UsageRatio` が閾値（例: 0.8）を超えたら Debug.LogWarning を出すテレメトリ、または開発ビルドで Assertion
