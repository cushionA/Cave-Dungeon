...
- [x] **Flinch 解除後 armor 復元仕様の確定** — PR #36 レビュー R3
  - 現状 (PR #36): Flinch 中被弾時に `vitals.currentArmor = 0f` を強制。Flinch 解除後も 0 のままで、既存の `UpdateArmorRecovery` で自然回復する
  - 仕様確定 (2026-04-26): 「Flinch 入り前の値復元」ではなく **「Flinch 解除瞬間に maxArmor へ満タンリセット」** で確定
  - 対象: `Assets/MyAsset/Runtime/Combat/DamageReceiver.cs:ForceZeroArmorIfInFlinch`
  - 優先度: 🟢 Minor (仕様判断次第)
  - ✅ `HitReactionLogic.ShouldResetArmorOnFlinchExit(prev, curr)` を新設 (Flinch → 非 Flinch 遷移判定)。`DamageReceiver._previousActState` で前フレーム ActState を保持し `Update()` で遷移検知 → `currentArmor = maxArmor` へ即時リセット。`ResetInternalState` でもリセット。EditMode 9 件 + PlayMode 4 件 (満タン復帰 / Flinch 継続中はリセットなし / Flinch → Knockbacked もリセット / Flinch 未経由は変更なし)
