...
- [ ] **BaseCharacter の GetComponent null フォールバック整理** — `BaseCharacter.Start` / `OnPoolAcquire` で `_damageReceiver` / `_animationController` が null の場合に `GetComponent` で再取得するフォールバックを追加した (PR #33)。これは `TestSceneHelper.CreateBaseCharacterObject` が `AddComponent<BaseCharacter>()` 後に `AddComponent<DamageReceiver>()` する順序依存を吸収するためのテスト向け妥協。本番プレハブでは Awake で取得できているので不要
  - 理想: TestSceneHelper 側で事前に全コンポーネントを追加する helper (`CreateBaseCharacterObjectWithDamageReceiver(info, position)`) に刷新し、`BaseCharacter` 側のフォールバックを削除
  - 対象: `Assets/Tests/PlayMode/TestSceneHelper.cs` / `Assets/MyAsset/Runtime/Character/BaseCharacter.cs:97-105,158-161`
  - 優先度: 🟢 Minor
