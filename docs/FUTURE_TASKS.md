...
- [ ] **justGuardResistance が BuildMotionData でコピーされない** — AttackInfo から AttackMotionData への変換で欠落し、ジャストガード時の軽減が常に0で機能しない
  - 対象: `Assets/MyAsset/Core/Common/CombatDataHelper.cs:17-40`
  - 修正: `BuildMotionData` に `justGuardResistance = info.justGuardResistance` を追加
...
- [ ] **属性別ガードカット率が未適用** — `GuardStats.slashCut/strikeCut/pierceCut/fireCut/thunderCut/lightCut/darkCut` 定義済みだが、DamageReceiver は一律 `GuardJudgmentLogic.GetDamageReduction(guardResult)` のみ使用
  - 対象: `Assets/MyAsset/Runtime/Combat/DamageReceiver.cs:248-286`
  - 修正: `attackElement` に応じて対応する `xxCut` を `DamageCalculator` に連携して属性チャネルごとに減算

- [ ] **ProjectileHitProcessor が DamageReceiver.ReceiveDamage を経由していない** — `HpArmorLogic.ApplyDamage` を直呼びしており、ガード/無敵/行動アーマー/SituationalBonus/ActionEffect.damageReduction/HitReaction/イベント発火を全スキップ
  - 対象: `Assets/MyAsset/Core/Combat/Projectile/ProjectileHitProcessor.cs:32-74`
  - 修正: 飛翔体ヒット時も `DamageReceiver.ReceiveDamage(damageData)` を呼ぶ経路に統一
