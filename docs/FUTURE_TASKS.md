...
WAVE_PLAN.md の Wave 2（主要 hook 群）は当初 4 PR 構成。PR #46 / #47 / #48 / #49 で 2 系列が完了したが、以下 2 系列が別ブランチで継続中。**Phase 10（Two-layer skill 試験）は `feature/wave2-phase10-two-layer` ブランチで現在実装中**のため、本セクションは Phase 10 完了後に再着手する Phase 11 残 / Phase 17 のリマインダーとして利用する。

- [ ] 🟡✓ **Wave 2 Phase 11 完結 — lint hook の error フェーズ昇格 + 成文化** — warn → error の段階昇格で Phase 11 を WBS 100% にする
  - **背景**: PR #47 で `tools/lint_check.py` + dispatcher の **warn フェーズ**が稼働中。WAVE_PLAN.md L713-715 の P11-T6/T7/T8 が未完。observation 期間を経て error に昇格する 2 段階運用を設計時から想定している
  - **仕様**:
    - **P11-T6（観察記録）**: PR #47 マージ（2026-04-25）から 1 週間の hook 出力を収集し、`.claude/rules/lint-observation-log.md` に集計。誤検知 3 件以上なら lint-patterns.json を調整
    - **P11-T7（lint.md 成文化）**: `.claude/rules/lint.md` を新設して「何が検査されるか / 誤検知時の抑止方法 / phase=warn → error 昇格条件」を成文化。`.claude/rules/README.md` のファイル一覧表に追記
    - **P11-T8（error 昇格 PR）**: `.claude/hooks/post-edit-dispatch.sh` または `.claude/hooks/lint-check.sh` で `LINT_PHASE=error` をデフォルト化。規約違反コードで PostToolUse が exit 1 を返す構成に切替（環境変数で warn に戻せる退避ルートも残す）
  - **対象ファイル**:
    - `.claude/hooks/post-edit-dispatch.sh`
    - `.claude/hooks/lint-check.sh`
    - `.claude/rules/lint.md`（新設）
    - `.claude/rules/lint-observation-log.md`（新設）
    - `.claude/rules/lint-patterns.json`（調整があれば）
    - `.claude/rules/README.md`（ファイル一覧追記）
  - **関連PR**: PR #47（warn フェーズ導入）
  - **ブランチ**: `feature/wave2-phase11-finish`
  - **実施タイミング**: 2026-05-02 以降（PR #47 マージ 2026-04-25 から 1 週間の観察期間後）

- [ ] 🟡✓ **Wave 2 Phase 17 — Registry-based handoff + Handoff Note 自動化** — セッション境界を超える知識転送を `docs/reports/_registry.md` で外在化
  - **背景**: WAVE_PLAN.md L719-730 の P17-T1〜T10。`designs/pipeline-state.json`（進行状態）+ `docs/compound/`（学習）と並ぶ**第三の外在化の柱**として、`docs/reports/` 体系で handoff note と各種レポートを索引化する。Anthropic 公式の effective context 戦略に準拠
  - **仕様**:
    - `docs/reports/` ディレクトリと `_registry.md` 索引を新設（カテゴリ: analysis / arch / bugs / handoffs / migrations / experiments / postmortems / reviews / research / specs / surveys 等 11 種）
    - 3 つの新規 skill:
      - `/handoff-note` — 手動生成（git diff main + 直近コミット + 未解決 TODO + 次セッションへの引き継ぎを `docs/reports/handoffs/<date>_<slug>.md` に出力）
      - `/resume-handoff` — 前回 handoff note を読み込み state を復元
      - `/registry-check` — `_registry.md` 一覧 + 直近 3 件の handoff を提示
    - `.claude/settings.json` に Stop hook（handoff note 自動生成）と SessionStart hook（`/registry-check` 起動）を追加
    - dream skill との競合回避: dream は `~/.claude/.dream-pending` で 24h ガード済み。handoff は毎セッションだが別ファイル名空間（`docs/reports/handoffs/` vs `~/.claude/.../memory/`）で衝突しない
    - 1 週間運用後に effective context 時間を計測、`docs/compound/` に効果測定エントリを追加
  - **対象ファイル**:
    - `docs/reports/_registry.md`（新設）+ 11 カテゴリ README.md
    - `.claude/skills/handoff-note/SKILL.md`（新設）
    - `.claude/skills/resume-handoff/SKILL.md`（新設）
    - `.claude/skills/registry-check/SKILL.md`（新設）
    - `.claude/settings.json`（Stop / SessionStart hook 追加）
  - **関連PR**: なし（新規）
  - **ブランチ**: `feature/wave2-phase17-handoff`
  - **実施タイミング**: Phase 11 残着手後 or Phase 10 PR マージ後（並行可）
