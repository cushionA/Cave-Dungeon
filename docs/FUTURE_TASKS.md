...
WAVE_PLAN.md の Wave 2（主要 hook 群）は当初 4 PR 構成。PR #46 / #47 / #48 / #49 で 2 系列が完了し、Phase 10 / Phase 17 / Phase 11 残（T7）の 3 PR で Wave 2 を完遂する見込み。**実時間が必要なタスクのみ本セクションに残す。**

- [ ] 🟡✓ **Wave 2 Phase 11 P11-T6 — lint hook の 1 週間誤検知観察記録** — error 昇格判定の材料を集計
  - **背景**: PR #47 で `tools/lint_check.py` + dispatcher の **warn フェーズ**が稼働中。`.claude/rules/lint.md` 成文化（P11-T7）と観察ログテンプレート（`lint-observation-log.md`）は本 PR で配置済み。残るは観察作業
  - **仕様**:
    - 2026-04-25（PR #47 マージ）〜 2026-05-02 の hook 出力（stderr）から `lint-observation-log.md` に findings を追記
    - `判定` 列で「真検知 / 誤検知 / 保留」を分類
    - 誤検知 3 件以上なら `lint-patterns.json` の該当 regex を調整
  - **対象ファイル**:
    - `.claude/rules/lint-observation-log.md` (本 PR で雛形配置済、観察期間中に追記)
    - `.claude/rules/lint-patterns.json` (誤検知パターン調整があれば)
  - **関連PR**: PR #47（warn フェーズ導入）/ 本 PR（成文化）
  - **実施タイミング**: 2026-04-25 〜 2026-05-02

- [ ] 🟡✓ **Wave 2 Phase 11 P11-T8 — lint hook の error フェーズ昇格 PR** — `LINT_PHASE=error` をデフォルト化
  - **背景**: P11-T6 観察期間後、誤検知 3 件以下を確認できたら error 昇格に進む
  - **仕様**:
    - `.claude/hooks/lint-check.sh` の `PHASE="${LINT_PHASE:-warn}"` を `${LINT_PHASE:-error}` に変更
    - 規約違反コードで PostToolUse が exit 1 を返す構成に切替
    - ロールバック手段: `export LINT_PHASE=warn` で warn に戻せる退避ルートを残す
    - PR 作成時は `lint.md` のロールバック手順を再掲
  - **対象ファイル**:
    - `.claude/hooks/lint-check.sh` (デフォルト phase 変更)
    - `.claude/rules/lint.md` (運用変更を反映)
  - **関連PR**: PR #47（warn 導入）/ 本 PR（成文化 + 観察ログ）
  - **昇格判定基準**: `lint-observation-log.md` の判定セクション全 3 項目が ✅ になっていること
  - **実施タイミング**: 2026-05-02 以降（観察期間終了後）

- [ ] 🟢✓ **Wave 2 Phase 17 P17-T9 — Registry-based handoff の 1 週間運用試験** — 効果測定 + compound エントリ作成
  - **背景**: Phase 17 の handoff registry 機能（registry / 3 skill / Stop & SessionStart hook）は実装完了見込み。残るは P17-T9 の運用試験のみ
  - **仕様**:
    - 2026-04-25 〜 2026-05-02 の間、handoff note を毎セッション末で作成（手動 or Stop hook 経由）
    - effective context 時間（前回作業を再開するまでの所要時間）を計測
    - 1 週間後に `docs/compound/2026-05-02-handoff-registry-trial.md` で効果測定エントリを作成
    - 改善点を `_registry.md` の運用ルールに反映
  - **対象ファイル**:
    - `docs/reports/handoffs/`（各セッションの note を蓄積）
    - `docs/reports/_registry.md`（索引更新）
    - `docs/compound/2026-05-02-handoff-registry-trial.md`（新設、計測結果）
  - **関連PR**: `feature/wave2-phase17-handoff`（実装本体）
  - **実施タイミング**: 2026-05-02 以降（1 週間運用後）

- ~~Wave 2 Phase 10（Two-layer skill 試験導入）~~ ✅ PR `feature/wave2-phase10-two-layer` で実装完了
- ~~Wave 2 Phase 11 P11-T7（lint.md 成文化）~~ ✅ 本 PR (`feature/wave2-phase11-finish`) で `.claude/rules/lint.md` + `lint-observation-log.md` 雛形を配置
- ~~Wave 2 Phase 17（Registry-based handoff の実装）~~ ✅ PR `feature/wave2-phase17-handoff` で実装完了（運用試験 P17-T9 のみ上記に残）
