...
- [x] **ヒット回数キャラ別カウント化** — HashSet→Dictionary<int,int>で同一キャラへの多段ヒット対応
  - 現状: HashSetで同一ターゲット1回制限
  - 変更: キャラごとにhitLimitまでヒット可能、非Pierceは最初のhitLimit到達で消滅
  - 対象: `ProjectileController.cs`
  - ✅ PR TBD で対応: `_hitCounts: Dictionary<int,int>` + `TryRegisterHit(targetHash, out shouldDespawn)` 追加。Pierce/非Pierce を `BulletProfile.features` から判定、非Pierce は hitLimit 到達で `Projectile.Kill()` を補完。`ProjectileHitCountTests` 新設

- [x] **弾丸サイズ変化** — 時間経過でコライダー・スプライトが拡大/縮小
  - BulletProfileにstartScale/endScale/scaleTime追加
  - ProjectileManager.UpdateでElapsedTime/scaleTimeのLerpで制御
  - ✅ PR TBD で対応 (弾丸拡張 4 項目の 1)

- [x] **出現・移動開始遅延** — 生成後N秒間は非表示・判定無しで待機（寿命は消費しない）
  - BulletProfileにspawnDelay追加
  - Projectile.Tickで遅延中はElapsedTimeを加算しない
  - ✅ PR TBD で対応 (弾丸拡張 4 項目の 2)

- [x] **弾丸スポーン位置オフセット** — スポーン位置からのローカルオフセット
  - BulletProfileにspawnOffset (Vector2) 追加
  - ✅ PR TBD で対応 (弾丸拡張 4 項目の 3)

- [x] **ターゲット位置スポーン** — TargetHashのSoA位置に直接出現する弾丸
  - 呼び出し側（MagicCaster等）がSoAから位置取得してSpawnProjectileに渡す
  - ✅ PR TBD で対応 (弾丸拡張 4 項目の 4)
...
- [x] **Coyote Time 未実装** — 接地から離れた瞬間にジャンプ不可（`GroundMovementLogic.TryStartJump` が `!isGrounded` で即return）
  - 対象: `Assets/MyAsset/Core/Movement/GroundMovementLogic.cs:64`
  - 修正: 最後に接地した時刻からN秒以内のジャンプ入力を許容するバッファを追加
  - ✅ PR TBD で対応

- [x] **壁蹴り呼び出し経路ゼロ** — `AdvancedMovementLogic.TryWallKick` は定義済みだが、PlayerCharacter / ActionExecutor からの呼び出し0件
...
- [x] **混乱解除時の即時AI再評価なし** — `JudgmentLoop.ForceEvaluate()` API + `GameEvents.OnConfusionCleared` ブロードキャストを追加。`ConfusionEffectProcessor(GameEvents)` コンストラクタで events 注入すれば `ClearConfusion` 時に `FireConfusionCleared` が発火
  - ✅ PR TBD で残タスク (EnemyController / CompanionController / BossController の AIBrain 側 subscribe → 自分のハッシュにマッチしたら `JudgmentLoop.ForceEvaluate()` を呼ぶ配線) を実装
...
- [x] **EnhancedGuard が JustGuardImmune 攻撃にも成立** → **仕様外として確定 / タスク取消**: PR #34 で確認済み、EnhancedGuard 機構自体は既に廃止済み。`GuardJudgmentLogic` には EnhancedGuard 判定が残っておらず、JustGuardImmune は通常のガード不可フラグとしてのみ扱う。
...
- [x] **カメラシェイクAPI未実装** — 被弾・爆発・着地強調のフィードバック不可
  - ✅ PR TBD で `CameraShakeController` を新設
