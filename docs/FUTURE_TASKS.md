...
WAVE_PLAN.md の Wave 3（品質強化）は Phase 5 / 6 / 13 / 22 の 4 PR 構成。本セクションは**実時間 / 実データ依存 / 外部判断依存** で自律消化できないタスクを集める。

### Phase 5 — スキル棚卸し運用化

- [ ] 🟢✓ **P5-T4 — 初回スキル棚卸し実施** — 使用頻度低スキルの archive 判定
  - **背景**: P5-T1〜T3 は `feature/wave3-phase5-skill-lifecycle` で実装完了見込み。`tools/skill-usage.py` を半年運用後の集計で初回棚卸しを実施
  - **仕様**: `python tools/skill-usage.py --since 180d` で半年スパンの集計を取得し、判定区分 `stale` / `candidate-archive` / `deprecated` を精査
  - **実施タイミング**: 2026-07-25 (制定 3 ヶ月後) または 2026-10-25 (半年後)

- [ ] 🟢✓ **P5-T5 — 棚卸し結果の反映** — archive / 削除コミット
  - **仕様**: `git mv .claude/skills/<name> .claude/skills/_archive/<name>` または `git rm -rf`
  - **実施タイミング**: P5-T4 直後

- [ ] 🟢⚠ **P5-T2 改善: skill-usage.py の deprecated 誤検知抑止**
  - **背景**: 初版実装で SKILL.md 本文の「廃止」「非推奨」「deprecated」「obsolete」キーワード単純検出により、これらを語る skill (`writing-skills`、`playtest` 等) を誤って deprecated 判定する
  - **仕様**: frontmatter `description:` 行のみ対象 / 行頭見出しのみ対象 / 専用 `deprecated: true` frontmatter フィールド導入のいずれか
  - **対象ファイル**: `tools/skill-usage.py:is_deprecated`
  - **実施タイミング**: P5-T4 実施前

### Phase 6 — TDD-Guard Hook 本格導入

- [ ] 🟡⚠ **Phase 6 全体 — `nizos/tdd-guard` の clone + PreToolUse hook 登録**
  - **背景**: 外部リポジトリ依存 + 既存編集ワークフローへの破壊的変更のため、ユーザー承認後に別 PR で実施
  - **仕様**: WAVE_PLAN.md L749-760 (P6-T1〜T8) を参照
  - **ブロッカー**: ユーザーの破壊的変更承認、外部 skill のライセンス確認、1 週間試用期間
  - **実施タイミング**: ユーザー承認後

### Phase 13 — TDD 3 サブエージェント分離

- [ ] 🟡✓ **P13-T7 — 1 機能を 3 分割版で実装試験** — Unity Edit Mode test 適合性確認
  - **背景**: P13-T1〜T6 は `feature/wave3-phase13-tdd-subagents` で実装する。残るは実機試験
  - **仕様**: 1 機能を新ワークフローで実装し、誤検知 / フェイル率を計測
  - **見積**: 2-3 時間 (実時間)
  - **実施タイミング**: P13 PR マージ後

### Phase 22 — Unity 特化 hook と skill 輸入

- [ ] 🟡⚠ **P22-T5〜T7 — 外部 skill 輸入** — Unity App UI Plugin / TheOne Studio skill の評価と採用
  - **背景**: 外部リポジトリのライセンス・C# 9 規約整合性・Architect/ 整合性のチェックが必要
  - **仕様**: WAVE_PLAN.md L783-786 (P22-T5〜T7) を参照
  - **ブロッカー**: 採否判断はユーザー、ライセンス精査が必要
  - **実施タイミング**: ユーザー判断後
