...
- [ ] **弾丸サイズ変化** — 仕様確定 (2026-04-23 レビュー)
  ...
  - **パフォーマンス最適化**: `scaleTime == 0` または `startScale == endScale` の場合は Update 処理を早期 return。大量弾幕時の不要な Lerp 呼び出しを回避
...
- [ ] **追尾力の時間経過変動** — 仕様確定 (2026-04-23 レビュー)
  - 現状: `ProjectileMovement.cs:21` で `homingStrength=5f` ハードコード
  - 仕様: `BulletProfile.homingStrength` (float, デフォルト5) + `homingAcceleration` (float)
  - 実効値: `homingStrength + homingAcceleration * ElapsedTime`
  - 強化（正値）/減衰（負値）両対応、曲線・上限値は実装者任せ (推奨: Linear / 上限20程度クランプ)
...
- [ ] **Coyote Time 未実装** — 仕様確定 (2026-04-23 レビュー、詳細は実装者任せ)
  - 現状: 接地から離れた瞬間にジャンプ不可 (`GroundMovementLogic.TryStartJump` が `!isGrounded` で即return)
  - 仕様: 最後に接地した時刻から N秒以内のジャンプ入力を許容するバッファ追加
    - 推奨: `GroundMovementLogic._lastGroundedTime` + `CharacterInfo.coyoteTime` (デフォルト 0.1s 程度)
    - Wall Slide 中の扱い、`jumpBufferTime` との役割分担は実装者判断 (※項目22で重複整理予定)
  - 対象: `Assets/MyAsset/Core/Movement/GroundMovementLogic.cs:64`

- [ ] **壁蹴り呼び出し経路ゼロ** — 仕様確定 (2026-04-23 レビュー)
  - 現状: `AdvancedMovementLogic.TryWallKick` は定義済みだが、呼び出し元0件
  - 仕様:
    - **キャラ共通モジュール**として `TryWallKick` を利用 (プレイヤー・敵・仲間すべて)
    - 実際に使えるかは `AbilityFlag.WallKick` + 各キャラのAI/コントローラー設計次第
    - **左右交互にしか蹴れない** (同じ側の壁は連続で蹴れない。左壁→右壁→左壁 の順で無限に蹴れるが、左壁→左壁 は不可)
      - 判定用: `_lastWallSide` (enum Left/Right) または `_lastWallNormalX` (符号) を保持し、反対側のみ許可
      - 補助: `_lastWallColliderId` (int) も併せて保持すると同一壁二度蹴り防止のデバッグに使える
    - **壁接触判定**: 既存 `CharacterCollisionController` の実装パターンに合わせる（Raycast/BoxCast等の選択は `CharacterCollisionController` の方針に従う）
    - 空中ジャンプ/ダッシュ連携は実装者任せ
  - 対象: `Assets/MyAsset/Core/Movement/AdvancedMovementLogic.cs:23` / `PlayerCharacter.cs` / `EnemyCharacter.cs` / `CompanionCharacter.cs` / `CharacterCollisionController.cs`
...
- [x] **混乱解除時の即時AI再評価なし** (基盤実装完了) — `JudgmentLoop.ForceEvaluate()` API + `GameEvents.OnConfusionCleared` ブロードキャストを追加。`ConfusionEffectProcessor(GameEvents)` コンストラクタで events 注入すれば `ClearConfusion` 時に `FireConfusionCleared` が発火
  - 残タスク: EnemyController / CompanionController / BossController の AIBrain 側で `GameEvents.OnConfusionCleared` を subscribe し、自分のハッシュにマッチしたら `JudgmentLoop.ForceEvaluate()` を呼ぶ配線
  - 優先度: 🟢 Low（Core 層の API 整備は完了、後は AIBrain 接続のみ）
...
- [ ] **Guard 系フラグ仕様整理** — 仕様確定 (2026-04-23 レビュー、元「EnhancedGuard が JustGuardImmune 攻撃にも成立」)
  - **`EnhancedGuard` フラグは廃止** (現コードから削除する)
  ...
  - 対象: `Assets/MyAsset/Core/Guard/GuardJudgmentLogic.cs:73-79` / `AttackFeature` enum 定義 / `EnhancedGuard` 参照箇所削除
...
- [ ] **Action armor 追加仕様** — 残タスク (2026-04-23 レビュー決定分)
  - 削り切った瞬間 (`actionArmor <= 0`) で**行動中断 → Flinch 遷移** を実装
  - **スーパーアーマー中はアーマーブレイクしない** (SuperArmor フラグで完全保護)
  - 対象: `Assets/MyAsset/Runtime/Combat/DamageReceiver.cs` / `ActionEffectProcessor`
...
- [ ] **Flinch中の挙動仕様** — 残タスク (2026-04-23 レビュー決定分、追撃耐性なし)
  - Flinch中は**完全に無防備**:
    - アーマー値もゼロに強制 (actionArmor / baseArmor 両方)
    - 再度怯み (Flinch上書き) は**しない** (タイマー再開や延長もしない)
    - **吹き飛ばし攻撃 (Knockback) だけは食らう** (Flinch→Knockback 遷移)
  - 対象: `DamageReceiver.cs:220-226` / `HitReactionLogic.cs`
...
- [ ] **カメラシェイクAPI未実装** — 方針確定 (2026-04-23 レビュー): **ProCamera2D 導入後に実装**
  - 被弾・爆発・着地強調のフィードバック用
  - ProCamera2D アセット（未導入）の Shake 機能を利用する前提
  - 呼び出しトリガー・強度テーブル・トグル設定等はProCamera2D導入後に設計
...
- [ ] **動的カメラBounds切替未実装** — 方針確定 (2026-04-23 レビュー): **ProCamera2D 導入後に実装**
  ...
  - 即時/スムーズ補間、Marker 配置方法等は導入後に決定

- [ ] **Look-Ahead未実装** — 方針確定 (2026-04-23 レビュー): **ProCamera2D 導入後に実装**
  - プレイヤー移動方向に先行してカメラを動かす機能
  - ProCamera2D の Look-Ahead 機能を利用
  - 追従速度・静止時戻り・向き変更時の補間等は導入後に調整

- [ ] **坂道法線取得なし** — 仕様確定 (2026-04-23 レビュー)
  - 現状: 接地判定は Y座標比較のみ (`CharacterCollisionLogic.CheckGrounded`)、法線未取得
  - 仕様:
    - 接地判定を **Raycast/BoxCast** に刷新 (`Physics2D.Raycast(origin, Vector2.down, rayLength, groundLayer)`)
    - `RaycastHit2D.normal` を `GroundInfo.normal` として保持
    - `GroundMovementLogic.UpdateMovement` で velocity を normal に沿って射影
    - **最大登坂角度 45°** 採用 (`Vector2.Angle(normal, Vector2.up) > 45f` で滑り落ち)
    - 滑り落ち挙動・坂ジャンプ方向・既存 CollisionLogic の置換/併存 は実装者任せ
  - 対象: `Assets/MyAsset/Core/Collision/CharacterCollisionLogic.cs:196-199` / `GroundMovementLogic.cs`

- [ ] **一方通行プラットフォーム（drop-through）未実装** — 仕様確定 (2026-04-23 レビュー、詳細は実装者任せ)
  - 下入力＋ジャンプで床を抜ける機能
  - 推奨: `PlatformEffector2D` + プレイヤーのレイヤー一時切替方式
  - 識別方法 (Effector有無 / 専用レイヤー / タグ)、復帰タイミング、敵AI対応は実装者判断

- [ ] **GroundMovementLogic の移動定数がデータ駆動化されていない** — 仕様確定 (2026-04-23 レビュー)
  ...
  - 仕様:
    ...
    - **`CharacterInfo.jumpBufferTime` と `minJumpHoldTime` は本来別概念なので両方残す**
      - `jumpBufferTime`: **着地前入力受付時間** (空中でジャンプ入力→着地後に自動発動の猶予)
      - `minJumpHoldTime`: **最小ホールド時間** (ジャンプボタン即離ししても最低限ジャンプさせる時間)
      - 当初は統合案だったが、役割が明確に異なるため実装時に両方キャラ別フィールドとして `CharacterInfo` に残す
    - デフォルト値管理方法 (SO / `MoveParams.Default`) は実装者任せ
...
- [x] **LevelUpLogic に最大レベルキャップなし** — `k_MaxLevel = 99` 追加。AddExp の while 条件に `_level < k_MaxLevel`、到達時は余剰 exp を破棄
  - 残タスク (仕様確定 2026-04-23 レビュー):
    - [ ] **ステータスポイント refund API (振り直し)** — **特殊アイテム消費**で発動（ゴールド/無料ではない）。コスト・回数制限は別途設計
    - [ ] **`k_MaxLevel` の ScriptableObject 化** — 今回は **`k_MaxLevel` のみ** SO 外出し（`LevelUpConfig` SO）。他の定数 (`k_PointsPerLevel`, `k_HpPerVit` 等) はハードコードのまま維持
      - 役割: **ハードキャップ**として機能。下記「動的算出値」がこの値を**超えないことを保証**する
    - [ ] **各ステータス (Str/Dex/Intel/Vit/Mnd/End) の上限値設定** — **各パラメータごとに上限値**を設定し、**動的算出**: `dynamicMaxLevel = パラメータ数 × 各上限値の合計 / pointsPerLevel`
      - 例: 6パラメータ × 上限99 / 3pts = 動的最大レベル198（上限構成次第で変動）
      - **実効最大レベル = `Min(dynamicMaxLevel, LevelUpConfig.k_MaxLevel)`** — SO のハードキャップが上限を規制
      - `AllocatePoint` は各ステータスの上限値で個別バリデーション（例: Str が cap に到達したら Str には振れない）
      - `CharacterInfo.statCaps` または新規 `LevelUpConfig` SO に上限配列を持たせる
