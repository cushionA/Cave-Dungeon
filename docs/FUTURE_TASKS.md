...
- [x] **クリティカル機構が未接続** → **仕様外として確定**: クリティカルヒット機構は本プロジェクトでは採用しない。`DamageCalculator.IsCritical` / `ApplyCritical` API は互換のため残置するが DamageReceiver からは呼ばない。`CombatStats.criticalRate` / `criticalMultiplier` フィールドは未使用の残骸として一時残置 (削除は将来のクリーンアップで検討)
